...
#[cfg(test)]
mod tests {
    use lexer::{Lexer, TokenPosition};
    use token::Token;
    use std::iter::Iterator;
    macro_rules! matchseq {
        ($lex:expr,$($a:expr),+) => {$(
            assert_eq!(Iterator::next(&mut $lex as &mut Lexer).unwrap().0,Token::from($a));
        )+};
    }
    #[test]
    fn lex_op_dot() {
    ...
    fn lex_ident() {
        ...
        let mut lex = Lexer::new(&src);
        assert_eq!(next(&mut lex).0, Token::Ident(format!("hello")));
        assert_eq!(next(&mut lex).0, Token::Ident(format!("world")));
    }
    #[test]
    fn lex_num_dec_int() {
        let src = format!("1234");
        let mut lex = Lexer::new(&src);
        assert_eq!(next(&mut lex).0, Token::Number(1234f64));
    }
    #[test]
    fn lex_num_dec_int_exp() {
        let src = format!("1234E31");
        let mut lex = Lexer::new(&src);
        assert_eq!(next(&mut lex).0, Token::Number(1234E31f64));
    }
    #[test]
    #[should_panic]
    ...
    fn lex_num_dec_float() {
        ...
        let mut lex = Lexer::new(&src);
        assert_eq!(next(&mut lex).0, Token::Number(1.234f64));
    }
    #[test]
    fn lex_num_hex_int() {
        let src = format!("0xFFFF");
        let mut lex = Lexer::new(&src);
        assert_eq!(next(&mut lex).0, Token::Number(65535f64));
    }
    #[test]
    #[should_panic]
    fn lex_num_hex_eof() {
    ...
    fn lex_str() {
        let src = format!("\"Hello, '\\\"world!\\\"'\"\n'ayoo\\a'");
        let mut lex = Lexer::new(&src);
        matchseq!(lex, Token::StaticString(format!("Hello, '\"world!\"'")));
        matchseq!(lex, Token::StaticString(format!("ayoo\x07")));
    }
    #[test]
    fn lex_general() {
    ...
    fn next(lex: &mut Lexer) -> (Token, TokenPosition) {
        Iterator::next(lex).unwrap()
    }
}
